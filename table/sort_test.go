...
func TestSortSingleColumnAscAndDesc(t *testing.T) {
	...
	assertOrder := func(expectedList []string) {
		for index, expected := range expectedList {
			idVal, ok := model.GetRows()[index].Data[idColKey]
	...
	assert.Len(t, model.GetRows(), len(rows))
...
func TestSortTwoColumnsAscDescMix(t *testing.T) {
	...
	assertVals := func(index int, name string, score int) {
		actualName, ok := model.GetRows()[index].Data[nameKey].(string)
		assert.True(t, ok)

		actualScore, ok := model.GetRows()[index].Data[scoreKey].(int)
		assert.True(t, ok)

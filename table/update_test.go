...
func TestSelectingRowWhenTableUnselectableDoesNothing(t *testing.T) {
	...
	assert.False(t, model.GetRows()[0].selected, "Row shouldn't be selected to start")

	model, _ = model.Update(tea.KeyMsg{Type: tea.KeyEnter})

	assert.False(t, model.GetRows()[0].selected, "Row shouldn't be selected after key press")
}
...
func TestSelectingRowToggles(t *testing.T) {
	...
	assert.True(t, model.GetRows()[0].selected, "Row should be selected after first toggle")

	model, _ = model.Update(keyEnter)
	assert.False(t, model.GetRows()[0].selected, "Row should not be selected after second toggle")

	model, _ = model.Update(keyDown)
	model, _ = model.Update(keyEnter)
	assert.True(t, model.GetRows()[1].selected, "Second row should be selected after moving and toggling")
}
